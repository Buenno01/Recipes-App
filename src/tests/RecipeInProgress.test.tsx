...
const firstDrinkClick = JSON.stringify({
  drinks: { 17256: ['Goldschlager'] },
});

const secondDrinkClick = JSON.stringify({
...
describe('RecipeInProgress', () => {
  ...
  it('should save the progress of a recipe properly', async () => {
    ...
    expect(storageSpy).toHaveBeenLastCalledWith('inProgressRecipes', firstDrinkClick);

    await userEvent.click(ingredient1);

    expect(storageSpy).toHaveBeenLastCalledWith('inProgressRecipes', secondDrinkClick);
  });
  ...
  it('should disable the finish button until all ingredients are checked', async () => {
    ...
    expect(screen.getByTestId(finishRecipeBtnTestId)).toBeEnabled();
  });
